...
class EvalConfig(Config):
    ...
    CHECKPOINT_PATH1 = (
        "checkpoints/salf_cbm_final_for_graph/save_graph_model_stage1.pth"
    )
    # RET-CLIP/checkpoints/salf_cbm_final_for_graph/stage3_spatial_graph.pth
    CHECKPOINT_PATH2 = (
        "checkpoints/salf_cbm_final_for_graph_moreEpoch/save_graph_model_stage2.pth"
    )
    ...
    if bootstrap_eval:
        ...
        for concept in CONCEPT_COLUMNS:
            if len(boot_metrics["auc"][concept]) > 0:
                print(
                    f"{concept:<4}: {np.mean(boot_metrics['auc'][concept]):.4f} ± {np.std(boot_metrics['auc'][concept]):.4f}"
                )
...
def visualize_full_report(data_file="full_eval_results.npy"):
    ...
    for i, concept in enumerate(CONCEPT_COLUMNS):
        ...
        if len(np.unique(y_t)) > 1:
            ...
            ax2.plot(
                x_smooth,
                np.clip(t_smooth, 0, 1),
                label=f"{concept} ({current_auc:.3f})",
            )
    ...
    if data["boot_metrics"] is not None:
        ...
        means = [
            np.mean(boot["auc"][c]) for c in CONCEPT_COLUMNS if len(boot["auc"][c]) > 0
        ]
        stds = [
            np.std(boot["auc"][c]) for c in CONCEPT_COLUMNS if len(boot["auc"][c]) > 0
        ]
...
def visualize_samples_by_grade(model, dataset, samples_per_grade, save_dir, device):
    ...
    with torch.no_grad():
        for i, idx in enumerate(tqdm(selected_indices, desc="Plotting")):
            ...
            ax.set_title(
                f"ID: {img_id}\nTrue Grade: {true_grade} | Pred: {pred_grade}",
                color=color,
                fontweight="bold",
            )
            ax.axis("off")
            ...
            for c_idx in range(6):
                ...
                heatmap_resized = F.interpolate(
                    heatmap_tensor, size=(224, 224), mode="bicubic", align_corners=False
                )
            ...
            save_path = os.path.join(
                save_dir, f"Grade{true_grade}_Pred{pred_grade}_{img_id}.png"
            )
...
# ==========================================
def visualize_raw_matrices_by_grade(
    model, dataset, samples_per_grade, save_dir, device
):
    print("\n" + "=" * 50)
    ...
    with torch.no_grad():
        for i, idx in enumerate(tqdm(selected_indices, desc="Plotting Raw Grids")):
            ...
            save_path = os.path.join(
                grid_save_dir,
                f"RawGrid_Grade{true_grade}_Pred{pred_grade}_{img_id}.png",
            )
...
# ==========================================
def visualize_grid_overlays_by_grade(
    model, dataset, samples_per_grade, save_dir, device
):
    print("\n" + "=" * 50)
    ...
    with torch.no_grad():
        for i, idx in enumerate(tqdm(selected_indices, desc="Plotting Grid Overlays")):
            ...
            save_path = os.path.join(
                overlay_save_dir,
                f"GridOverlay_Grade{true_grade}_Pred{pred_grade}_{img_id}.png",
            )
            # 缩小边缘留白，不使用 tight_layout 防止挤压 colorbar
            plt.subplots_adjust(left=0.05, right=0.9, top=0.85, bottom=0.1, wspace=0.1)
            plt.savefig(save_path, dpi=150)
...
def visualize_grid_overlays_by_grade_compare(
    model, dataset, samples_per_grade, save_dir, device
):
    print("\n" + "=" * 50)
    ...
    with torch.no_grad():
        for i, idx in enumerate(tqdm(selected_indices, desc="Plotting Grid Overlays")):
            ...
            pred_probs_post = (
                torch.sigmoid(lesion_logits_graph).squeeze(0).cpu().numpy()
            )
            ...
            for row_idx, (row_name, pred_g, pred_probs, maps_vis) in enumerate(
                rows_data
            ):
            ...
            plt.subplots_adjust(
                left=0.05, right=0.9, top=0.9, bottom=0.1, wspace=0.1, hspace=0.3
            )
            plt.savefig(save_path, dpi=150)
            plt.close()
...
def visualize_grading_report(
    data_path="full_eval_results.npy", save_name="grading_report.png"
):
    """
...
def visualize_lesion_report(
    data_path="full_eval_results.npy", save_name="lesion_report.png"
):
    """
    ...
    fig.suptitle(
        "Lesion Concept Detection Performance Report",
        fontsize=18,
        fontweight="bold",
        y=0.98,
    )
    ...
    # 设置 ax1 样式 (ROC)
    ax1.plot(
        [0, 1], [0, 1], color="navy", lw=1.5, linestyle="--", label="Random Guess (0.5)"
    )
    ...
    ax1.set_title(
        "Subplot A: Smoothed & Corrected ROC Curves",
        fontsize=15,
        fontweight="bold",
        pad=10,
    )
    ...
    if boot is not None and "auc" in boot:
        ...
        if valid_labels:
            ...
            ax2.set_title(
                "Subplot B: Bootstrap Performance Deviation (Mean ± Std)",
                fontsize=15,
                fontweight="bold",
                pad=10,
            )
            ...
            ax2.legend(
                handles=[patch_pos, patch_neu, patch_neg], loc="upper right", fontsize=9
            )
...
def visualize_comprehensive_grading(data_path="full_eval_results.npy", num_classes=5):
    ...
    print(
        f"Metrics -> ACC: {acc:.4f}, AUC: {macro_auc:.4f}, AUPR: {macro_aupr:.4f}, F1: {macro_f1:.4f}, Kappa: {kappa:.4f}"
    )
    ...
    fig.suptitle(
        "Diabetic Retinopathy Grading - Comprehensive Performance",
        fontsize=16,
        fontweight="bold",
    )
    ...
    for i in range(num_classes):
        ...
        ax1.plot(
            ...
            color=colors_line[i],
            lw=2,
            label=f"Grade {i} (AUC = {class_auc:.3f})",
        )
    ...
    for i in range(num_classes):
        ...
        ax2.plot(
            ...
            color=colors_line[i],
            lw=2,
            label=f"Grade {i} (AUPR = {class_aupr:.3f})",
        )
    ...
    # plt.show()
    print(
        "[Success] Comprehensive grading report saved as 'grading_comprehensive_metrics.png'"
    )
...
def visualize_comprehensive_grading_with_baseline(
    data_path="full_eval_results.npy", num_classes=5
):
    """
    ...
    fig.suptitle(
        "Diabetic Retinopathy Grading - Comprehensive Performance (with AUPR Baselines)",
        fontsize=16,
        fontweight="bold",
    )
    colors_line = plt.cm.get_cmap("Set1")(np.linspace(0, 1, num_classes))
    ...
    for i in range(num_classes):
        ...
        ax2.axhline(
            y=class_baselines[i], color=colors_line[i], linestyle=":", alpha=0.7, lw=1.5
        )
    ...
    ax3.axhline(
        y=macro_baseline, color="#9b59b6", linestyle="--", alpha=0.8, xmin=0.4, xmax=0.6
    )
    ax3.text(
        2,
        macro_baseline + 0.02,
        f"Macro Base: {macro_baseline:.3f}",
        color="#8e44ad",
        ha="center",
        fontsize=9,
        fontweight="bold",
