...
适配最新的 MIL-VT 实现（带 Cross-Attention 和分级头）

功能：
1. 按分级抽样展示 MIL-VT 生成的概念图
2. 展示 Cross-Attention 的相似度分数
3. 网格叠加效果（14x14 马赛克）
4. 展示分级预测结果
"""
...
class VisConfig:
    ...
    MIL_VT_CHECKPOINT = "checkpoints/mil_vt/best_mil_vt.pth"

    SAVE_DIR = "evaluation_results/mil_vt_visualizations_v2"
    SAMPLES_PER_GRADE = 3
...
def visualize_mil_vt_grid_overlays(model, dataset, samples_per_grade, save_dir, device):
    ...
    with torch.no_grad():
        for i, idx in enumerate(tqdm(selected_indices, desc="Plotting")):
            ...
            # 模型推理
            concept_maps, lesion_logits, similarity_scores = model(
                image_tensor,
            ...
            sim_scores = similarity_scores.squeeze(0).cpu().numpy()  # [6, 196]

            # --- 开始画图 ---
            # 布局: 1张原图 + 6张概念网格叠加 + 6张相似度热力图
            ...
            for c_idx in range(6):
                ...
                # 将 [196] 重塑为 [14, 14]
                sim_map = sim_scores[c_idx].reshape(14, 14)
                ...
                # 找到最大值位置（最相关的 Patch）
                max_pos = np.unravel_index(sim_map.argmax(), sim_map.shape)
                ax.plot(max_pos[1], max_pos[0], 'c*', markersize=15, markeredgecolor='white', markeredgewidth=1)
            ...
            fig.colorbar(im_sim, cax=cbar_ax2, label='Similarity Score')

            # 保存
            save_path = os.path.join(
                save_dir,
                f"MIL_VT_Grade{true_grade}_{img_id}.png"
            )
...
def main():
    ...
    # 生成可视化
    print("\n" + "="*70)
    print(" "*20 + "MIL-VT VISUALIZATION V2")
    print("="*70)
