...
class SALF_CBM_Fusion(nn.Module):
    def __init__(
        ...
        self.clip_model = create_model(
            "ViT-B-16@RoBERTa-wwm-ext-base-chinese", checkpoint=checkpoint
        )
        ...
        self.concept_projector = nn.Conv2d(
            in_channels=768, out_channels=self.num_concepts, kernel_size=1, bias=False
        ).to(self.device)
        ...
        # 4. 门控融合模块
        # ==========================================
        self.fusion_module = GatedFusionModule(num_concepts=self.num_concepts).to(
            self.device
        )
        ...
        self.spatial_graph = SpatialConceptGraph(
            num_concepts=self.num_concepts, pool_size=7
        ).to(device)
        ...
        self.aux_head = nn.Sequential(
            nn.Linear(concept_dim, 64), nn.ReLU(), nn.Linear(64, self.num_concepts)
        ).to(device)

        self.lesion_head = nn.Sequential(
            nn.Linear(fused_dim, 64), nn.ReLU(), nn.Linear(64, self.num_concepts)
        ).to(device)
    ...
    def _initialize_concept_projector_with_text(self):
        ...
        text_inputs = tokenizer(prompts, return_tensors="pt", padding=True).to(
            self.device
        )
        ...
        with torch.no_grad():
            ...
            self.concept_projector.weight.data = fused_weight.view(
                self.num_concepts, 768, 1, 1
            )
    ...
    def load_clip_weights(self, clip_checkpoint_path):
        ...
        clip_weights = {
            k.replace("concept_projector.", ""): v
            for k, v in checkpoint.items()
            if "concept_projector" in k
        }
        if clip_weights:
...
if __name__ == "__main__":
    ...
    BACKBONE_PATH = (
        "./checkpoints/finetuned_model/dr_grading_finetune/checkpoints/epoch_latest.pt"
    )

    try:
        model = SALF_CBM_Fusion(
            checkpoint_path=BACKBONE_PATH, concepts=CONCEPTS, device="cuda"
        )
        model.eval()
