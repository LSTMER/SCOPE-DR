...
def run_stage1_fusion_and_aux(model, train_loader, val_loader, cfg):
    ...
    optimizer = optim.AdamW([
        ...
        {'params': model.lesion_head.parameters(), 'lr': cfg.STAGE1_LR_AUX}
    ], weight_decay=1e-4)

    # pos_weights = torch.tensor([1.0, 1.0, 5.0, 1.0, 1.0, 1.0])
    # criterion_aux = nn.BCEWithLogitsLoss(pos_weight=pos_weights.to(cfg.DEVICE))
    ...
    for epoch in range(cfg.STAGE1_EPOCHS):
        ...
        if auc > best_score:
            best_score = auc
            torch.save(model.state_dict(), os.path.join(cfg.SAVE_DIR, "stage1_fusion_aux.pth"))
            print(f"★ Best Model Saved! (AUC: {auc:.4f})")

    print("★ Stage 1 Completed.")
    model.load_state_dict(torch.load(os.path.join(cfg.SAVE_DIR, "stage1_fusion_aux.pth")))
    return model
...
def run_stage2_initial_grading(model, train_loader, val_loader, cfg):
    ...
    for epoch in range(cfg.STAGE2_EPOCHS):
        ...
        if kappa > best_kappa:
            best_kappa = kappa
            torch.save(model.state_dict(), os.path.join(cfg.SAVE_DIR, "stage2_grading.pth"))
...
def run_stage3_graph_learning(model, train_loader, val_loader, cfg):
    ...
    for epoch in range(cfg.STAGE3_EPOCHS):
        ...
        if auc > best_auc:
            best_auc = auc
            torch.save(model.state_dict(), os.path.join(cfg.SAVE_DIR, "stage3_graph.pth"))
            print(f"★ Best Model Saved! (AUC: {auc:.4f})")
...
def run_stage4_final_grading(model, train_loader, val_loader, cfg):
    ...
    for epoch in range(cfg.STAGE4_EPOCHS):
        ...
        if kappa > best_kappa:
            best_kappa = kappa
            torch.save(model.state_dict(), os.path.join(cfg.SAVE_DIR, "stage4_final.pth"))
            print(f"★ Best Model Saved! (Kappa: {kappa:.4f})")
...
def main():
    ...
    if os.path.exists(cfg.CLIP_CHECKPOINT):
        model.load_state_dict(torch.load(cfg.CLIP_CHECKPOINT, map_location=cfg.DEVICE), strict=False)
